...
class STFTDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print('STFT Failed loading frame/audio: {}'.format(e))
            # create dummy data
            mag_mix, mags, frames, audios, phase_mix, audio_mix = \
    ...
    def __getitem__(self, index):
        ...
        try:
            for n, infoN in enumerate(infos):
                ...
                center_timeN = (center_frames[n] - 0.5) / self.fps
                audios[n] = self._load_audio(path_audios[n], center_timeN)
            audio_mix = self._mix_raw(audios)

        except Exception as e:
            print('RAW Failed loading frame/audio: {}'.format(e))
            mag_mix, mags, frames, audios, phase_mix, audio_mix = \
                self.dummy_mix_data(N)

        ret_dict = {'audio_mix': audio_mix, 'frames': frames, 'audios': audios}
        if self.split != 'train':
            ret_dict['infos'] = infos
